import { FOLLOW_UP_NEEDED_STATUS } from '../config/constants.js';
import { addDays, startOfPracticeDayInstant, startOfToday } from '../utils/date.js';

function dateFromOffset(offset: number | null): Date | null {
  if (offset === null) return null;
  return addDays(startOfToday(), offset);
}

function createdFromOffset(offset: number): Date {
  return startOfPracticeDayInstant(addDays(startOfToday(), offset));
}

export function buildSampleInquiries() {
  return [
    {
      name: 'Maya Johnson',
      phone: '404-555-0128',
      email: 'maya.johnson@example.com',
      service_needed: 'Spinal Adjustment',
      activity_context: 'Desk worker; lower back stiffness after long sitting.',
      source: 'Referral',
      status: 'Consultation Scheduled',
      estimated_value: 220,
      notes: 'Referred by an existing patient; lower back stiffness and prefers morning appointments.',
      next_follow_up_date: dateFromOffset(-3),
      appointment_status: 'Appointment Scheduled',
      patient_type: 'New Patient',
      appointment_request: 'Tuesday morning',
      offer_type: 'None',
      last_visit_date: null,
      expected_visit_frequency_days: null,
      assigned_follow_up_owner: 'Front Desk',
      follow_up_outcome: 'Spoke - Scheduled',
      created_at: createdFromOffset(-12),
    },
    {
      name: 'Andre Walker',
      phone: '678-555-0191',
      email: 'andre.walker@example.com',
      service_needed: 'Sports Injury Treatment',
      activity_context: 'Runner; wants to return to weekend training.',
      source: 'Google',
      status: 'New Inquiry',
      estimated_value: 280,
      notes: 'Runner with weekend knee and hip pain; asked about first-visit availability.',
      next_follow_up_date: dateFromOffset(1),
      appointment_status: 'Not Scheduled',
      patient_type: 'New Patient',
      appointment_request: 'First available after 4 PM',
      offer_type: 'None',
      last_visit_date: null,
      expected_visit_frequency_days: null,
      assigned_follow_up_owner: 'Front Desk',
      follow_up_outcome: 'Not Contacted',
      created_at: createdFromOffset(0),
    },
    {
      name: 'Priya Patel',
      phone: '470-555-0177',
      email: 'priya.patel@example.com',
      service_needed: 'Wellness Consultation',
      activity_context: 'Active adult; monthly mobility and maintenance goal.',
      source: 'Insurance',
      status: FOLLOW_UP_NEEDED_STATUS,
      estimated_value: 175,
      notes: 'Asked whether her insurance covers a wellness visit and wants a benefits check.',
      next_follow_up_date: dateFromOffset(-5),
      appointment_status: 'Not Scheduled',
      patient_type: 'Reactivation',
      appointment_request: '',
      offer_type: 'None',
      last_visit_date: dateFromOffset(-45),
      expected_visit_frequency_days: 30,
      assigned_follow_up_owner: 'Doc',
      follow_up_outcome: 'Left Voicemail',
      created_at: createdFromOffset(-18),
    },
    {
      name: 'Chris Miller',
      phone: '770-555-0142',
      email: 'chris.miller@example.com',
      service_needed: 'Neck Pain Evaluation',
      activity_context: 'Desk worker; neck tension affecting workdays.',
      source: 'Phone Call',
      status: 'Active Patient',
      estimated_value: 260,
      notes: 'Completed consult and started care plan.',
      next_follow_up_date: null,
      appointment_status: 'Appointment Scheduled',
      patient_type: 'Existing Patient',
      appointment_request: '',
      offer_type: 'None',
      last_visit_date: dateFromOffset(-40),
      expected_visit_frequency_days: 30,
      assigned_follow_up_owner: 'Front Desk',
      follow_up_outcome: 'Not Contacted',
      created_at: createdFromOffset(-30),
    },
    {
      name: 'Nia Brown',
      phone: '912-555-0184',
      email: 'nia.brown@example.com',
      service_needed: 'Prenatal Chiropractic Consultation',
      activity_context: 'Prenatal wellness; side-sleeping discomfort.',
      source: 'Website',
      status: 'Consultation Scheduled',
      estimated_value: 240,
      notes: 'Website inquiry asking about prenatal care options and side-sleeping discomfort.',
      next_follow_up_date: dateFromOffset(0),
      appointment_status: 'Appointment Scheduled',
      patient_type: 'New Patient',
      appointment_request: 'Thursday afternoon',
      offer_type: 'Other',
      last_visit_date: null,
      expected_visit_frequency_days: null,
      assigned_follow_up_owner: 'Front Desk',
      follow_up_outcome: 'Spoke - Scheduled',
      created_at: createdFromOffset(-4),
    },
    {
      name: 'Jordan Lee',
      phone: '706-555-0109',
      email: 'jordan.lee@example.com',
      service_needed: 'Back Pain Consultation',
      activity_context: 'Warehouse work; recurring back discomfort.',
      source: 'Referral',
      status: 'Lost',
      estimated_value: 190,
      notes: 'Chose another clinic closer to work after price comparison.',
      next_follow_up_date: null,
      appointment_status: 'Not Scheduled',
      patient_type: 'Dead Lead',
      appointment_request: '',
      offer_type: 'None',
      last_visit_date: dateFromOffset(-120),
      expected_visit_frequency_days: 30,
      assigned_follow_up_owner: '',
      follow_up_outcome: 'No Response',
      created_at: createdFromOffset(-21),
    },
    {
      name: 'Aisha Coleman',
      phone: '470-555-0120',
      email: 'aisha.coleman@example.com',
      service_needed: 'Spinal Adjustment',
      activity_context: 'Reactivation; wants evening visits to stay consistent.',
      source: 'Phone Call',
      status: FOLLOW_UP_NEEDED_STATUS,
      estimated_value: 230,
      notes: 'Called during lunch break; wants evening appointment options and needs callback.',
      next_follow_up_date: dateFromOffset(-1),
      appointment_status: 'Not Scheduled',
      patient_type: 'Reactivation',
      appointment_request: 'After 5 PM',
      offer_type: 'None',
      last_visit_date: dateFromOffset(-30),
      expected_visit_frequency_days: 30,
      assigned_follow_up_owner: 'Doc',
      follow_up_outcome: 'Not Contacted',
      created_at: createdFromOffset(-7),
    },
    {
      name: 'Ben Carter',
      phone: '706-555-0181',
      email: 'ben.carter@example.com',
      service_needed: 'Sports Injury Treatment',
      activity_context: 'Soccer player; recurring ankle issue.',
      source: 'Google',
      status: 'Consultation Scheduled',
      estimated_value: 275,
      notes: 'Soccer parent asking about recovery plan for recurring ankle issue.',
      next_follow_up_date: dateFromOffset(5),
      appointment_status: 'Appointment Scheduled',
      patient_type: 'Existing Patient',
      appointment_request: 'Saturday morning',
      offer_type: 'Groupon',
      last_visit_date: dateFromOffset(-20),
      expected_visit_frequency_days: 30,
      assigned_follow_up_owner: 'Front Desk',
      follow_up_outcome: 'Spoke - Scheduled',
      created_at: createdFromOffset(-2),
    },
  ].map((item) => ({ ...item, updated_at: item.created_at }));
}
